      ...
      <div className="mt-12 grid gap-5 sm:grid-cols-3">
        {ENTRIES.map((entry) => (
          <Link
            key={entry.href}
            href={entry.href}
            className="card group p-6 transition hover:-translate-y-1 hover:border-accent-line"
          >
            <div className="text-4xl">{entry.emoji}</div>
            <h2 className="mt-4 text-xl font-bold group-hover:text-accent">{entry.title}</h2>
